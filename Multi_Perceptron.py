...
class ClNeuralNetwork:
    ...
    def adjust_weights(self, input_samples, error):
        ...
        # When the samples are divided by the lines in the plane and
        # there is no error in the output return.
        if (np.all(error == 0)):
            return
        ...
        for layer_index, layer in enumerate(self.layers):
                ...
                """
                   Stack the input arrays to column to get the input tuples.
                """
                input_column_stack = np.vstack([input_samples, np.ones((1, input_samples.shape[1]), float)])
                input_column_stack = np.column_stack(input_column_stack)

                """
                    Get the corresponding error from the error array.
                """
                error_column_stack = np.vstack([error, np.ones((1, input_samples.shape[1]), float)])
                error_column_stack = np.column_stack(error_column_stack)

                """
                    Get Input tuple and error multiplier.
                """
                input_row = input_column_stack[self.input_index]
                error_row = error_column_stack[self.input_index]

                for input_num in range(layer.number_of_neurons):
                    """
                        Perform multiplication of error, input and learning rate.
                    """
                    error_input = np.multiply(error_row[input_num], input_row)
                    error_input = np.multiply(self.learning_rate, error_input)

                    """
                        Add the multiplication result to weight.
                    """
                    layer.weights[input_num] = np.add(layer.weights[input_num], error_input)

        """
            On reaching last input, restart from first input again.
        """
        self.input_index = (self.input_index + 1) % (self.experiment.number_of_classes * self.experiment.number_of_samples_in_each_class)
